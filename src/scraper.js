...
async function searchImages({ reference, title, brand }) {
  ...
  if (title && title.length > 5) {
    ...
    // Wrap the reference in quotes for an exact match to avoid fuzzy matching (e.g. KB2006 instead of KB2008)
...
async function _validateImageUrl(url) {
  try {
    ...
    // Accept OK or Partial Content. If server rejects Range requests, we give it the benefit of the doubt
    if (!res.ok && res.status !== 206) {
      if (res.status === 403 || res.status === 401 || res.status === 416 || res.status === 405) return true;
      return false;
    }
    
    const contentType = res.headers.get('content-type');
    // Reject definitively corrupted/HTML files
    if (contentType && (contentType.includes('text/html') || contentType.includes('application/json') || contentType.includes('xml'))) {
      return false;
    }
